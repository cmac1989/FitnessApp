...
const MessagesScreen = () => {
    ...
    const handleLike = useCallback(async (messageId) => {
        ...
        try {
            const res = await toggleTrainerMessageLike(messageId);
            setMessages(prev => prev.map(m =>
        ...
        Alert.alert(
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            await deleteTrainerMessage(messageId);
                            setMessages(prev => prev.map(m =>
            ...
            <>
                ...
                >
                    <View style={[styles.row, isMine ? styles.rowMine : styles.rowTheirs]}>
                        {!isMine && (
                            <Avatar name={client?.name} size={30} color={theme.accent} />
                        )}
    ...
    }, [myId, theme, styles, handleLongPress, messages, client]);
        ...
        return (
            <View style={styles.emptyContainer}>
                <View style={styles.emptyAvatar}>
                    <Text style={styles.emptyAvatarText}>{getInitials(client?.name)}</Text>
                </View>
                <Text style={styles.emptyTitle}>{client?.name ?? 'Client'}</Text>
...
const makeStyles = (theme) => StyleSheet.create({
    ...
    // Bubble wrapper (relative so reaction badge can be absolute)
    bubbleWrap: {
        maxWidth: '75%',
        position: 'relative',
        marginBottom: 10,
